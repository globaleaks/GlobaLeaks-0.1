...
ADDRESS="195.85.254.203"
NAME=globaleaks
CONFIG=/etc/glinstances.conf
PIDDIR=/var/run/$NAME
PASSWORD=""
DAEMON=/usr/bin/python
#DAEMON_ARGS="web2py.py -i $ADDRESS -p $PORT --password=$PASSWORD"
DAEMON_USER=globaleaks
DAEMON_DIR=/data/globaleaks-instances
...
do_start()
{
    ...
    for i in $INSTANCE; do
        instance_name=`expr match $i "\(.*\):.*"`
        instance_port=`expr match $i ".*:\(.*\)"`
        ...
        if [ ! -d $instance_dir"/"$instance_name ]; then
            ...
            start-stop-daemon --stop --test --quiet --pidfile \
                $pidfile && echo "Already running instance." && continue

            echo Starting GlobaLeaks instance $instance_name
            DAEMON_ARGS="web2py.py -i $ADDRESS -p $instance_port \
                --pid_filename=$pidfile --password=$PASSWORD"

            # launch the daemon
            start-stop-daemon --start --pidfile $pidfile \
                ${DAEMON_USER:+--chuid $DAEMON_USER} \
        --chdir $instance_dir/$instance_name/globaleaks \
